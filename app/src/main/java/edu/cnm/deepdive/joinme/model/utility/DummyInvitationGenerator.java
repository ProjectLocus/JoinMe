...
public class DummyInvitationGenerator {
   ...
   * @return the list
   */
  public static List<Invitation> getXDummyInvitations(boolean isReceivedInvite, UUID theUser, Context context, int xDummies){
    List<Invitation> dummyInvitations = new LinkedList<>();
    for (int i = 0; i < xDummies; i++) {
      Invitation tempInvitation = new Invitation();
      if(isReceivedInvite){
        tempInvitation.setUserSender(DummyPersonGenerator.getXDummyPersonsNoDeviceUser(1, context).get(0).getPersonId());
        ...
        tempInvitation.setUserReceiver(DummyPersonGenerator.getXDummyPersonsNoDeviceUser(1, context).get(0).getPersonId());
      ...
      tempInvitation.setId(UUID.randomUUID());
